<template>
    <div>
        <button class="unstyle" @click="sendVote">
...
<script>
    ...
    export default class Like extends Vue {
        @PropSync('questionId', { type: String }) syncedQuestionId
        ...
        async sendVote() {
            const response = await this.$voteService.sendDislike(this.syncedQuestionId);

            if (response.question.nModified === 1) {
                this.$emit('success-vote');
            }
        }
    }
</script>
